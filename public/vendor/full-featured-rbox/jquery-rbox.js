(function ($) {
    function createInitialElements() {
        ...
        var $lightboxBlock = $("<div />")
            .addClass("rbox-wrap")
            .appendTo($overlay);
        var $lightbox = $("<div />").addClass("rbox").appendTo($lightboxBlock);
        var $content = $("<div />")
            .addClass("rbox-content")
            .appendTo($lightbox);
        var $close = $("<a />")
            .attr({ href: "", "aria-label": "Close Button" })
            .addClass("rbox-close")
            .html("&#x274c;")
            .appendTo($lightbox);
        var $next = $("<a />")
            .attr({ href: "", "aria-label": "Next Button" })
            .addClass("rbox-next")
            .html("&#x25b6;")
            .appendTo($lightbox);
        var $prev = $("<a />")
            .attr({ href: "", "aria-label": "Previous Button" })
            .addClass("rbox-prev")
            .html("&#x25c0;")
            .appendTo($lightbox);
        $("body").append($overlay);
    ...
    $.fn.rbox = function (options) {
        ...
        return this.each(function () {
            ...
            var $this = $(this),
                dataOptions = $.extend(
                    options,
                    $this.getDataOptions(optionTypes, namespace)
                );
            var opts = $.extend(
                {
    ...
    function getLightBoxContent(opts, callback) {
        switch (opts.type) {
            ...
            case "iframe":
                ...
                var $content = $("<iframe />")
                    .attr({ src: opts.iframe })
                    .css({ height: aspectratio + "vw" });
                if (opts.width) {
                    $content.attr("width", opts.width);
                }
                if (opts.height) {
    ...
    function showLightbox(content, opts) {
        ...
        if (opts.bgcustominner) {
            $(".rbox-wrap--inline, .rbox-wrap--html, .rbox-wrap--ajax").css({
                background: opts.bgcustominner,
            });
        }
        ...
        $(window).resize(function () {
            if ($(window).height() < $(".rbox").height()) {
                ...
                $(".rbox-overlay")
                    .removeClass("rbox-overlay--short")
                    .css({ height: "" });
            }
        });
        $(window).resize();
    ...
    */
    $.fn.getDataOptions = function (optionTypes, namespace) {
        ...
        $.each(optionTypes["strings"], function (i, v) {
            var attr = prefix + v;
            options[v] = $element.hasAttr(attr)
                ? $element.attr(attr)
                : undefined;
        });
        $.each(optionTypes["integers"], function (i, v) {
            var attr = prefix + v;
            options[v] = $element.hasAttr(attr)
                ? parseInt($element.attr(attr))
                : undefined;
        });
        $.each(optionTypes["floats"], function (i, v) {
            var attr = prefix + v;
            options[v] = $element.hasAttr(attr)
                ? parseFloat($element.attr(attr))
                : undefined;
        });
        $.each(optionTypes["arrays"], function (i, v) {
            var attr = prefix + v;
            options[v] = $element.hasAttr(attr)
                ? $.map($element.attr(attr).split(","), $.trim)
                : undefined;
        });
