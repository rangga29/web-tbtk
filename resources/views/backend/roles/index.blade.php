...
@section('content')
    <nav class="page-breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page"><a href="{{ route('dashboard.roles.index') }}">Data Role</a></li>
        </ol>
    </nav>

    <div class="d-flex justify-content-between align-items-center grid-margin flex-wrap">
        <div>
            <h3 class="mb-md-0 fw-bolder text-uppercase mb-3">Data Role</h3>
        </div>
        @can('role-create')
            <div class="d-flex align-items-center text-nowrap flex-wrap">
                <a href="{{ route('dashboard.roles.create') }}" class="btn btn-primary btn-icon-text mb-md-0 mb-2">
                                ...
                                @foreach ($roles as $role)
                                    <tr>
                                        ...
                                        @canany(['role-edit', 'role-delete', 'role-permission'])
                                            <td>
                                                @can('role-edit')
                                                    <a href="{{ route('dashboard.roles.show', $role->id) }}" class="btn btn-primary btn-icon btn-xs me-1" title="GIVE REVOKE PERMISSION">
                                                        <i data-feather="check-square"></i>
                                                    </a>
                                                @endcan
                                                @can('role-edit')
                                                    <a href="{{ route('dashboard.roles.edit', $role->id) }}" class="btn btn-warning btn-icon btn-xs me-1" title="UBAH DATA">
