                            ...
                            <tbody>
                                @foreach ($posts as $post)
                                    <tr>
                                        ...
                                        <td>{{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $post->created_at)->isoFormat('DD MMMM Y HH:MM') }}</td>
                                        <td>{{ $post->isPublish == 0 ? 'Belum Publish' : 'Publish' }}</td>
                                        <td>{{ $post->isPublish == 1 ? $post->user_publish->name : '--' }}</td>
                                        <td>{{ $post->isPublish == 1 ? \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $post->publish_at)->isoFormat('DD MMMM Y HH:MM') : '--' }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
