...
@section('content')
    <nav class="page-breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page"><a href="{{ route('dashboard.post.category.index') }}">Data Kategori</a></li>
        </ol>
    </nav>
    ...
    <div class="d-flex justify-content-between align-items-center grid-margin flex-wrap">
        ...
        @can('post-category-create')
            <div class="d-flex align-items-center text-nowrap flex-wrap">
                <a href="{{ route('dashboard.post.category.create') }}" class="btn btn-primary btn-icon-text mb-md-0 mb-2">
                            ...
                            <tbody>
                                @foreach ($categories as $category)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('dashboard.post.category.show', $category->slug) }}">{{ $category->name }}</a>
                                        </td>
                                        <td>{{ $category->slug }}</td>
                                        @canany(['post-category-edit', 'post-category-delete'])
                                            <td>
                                                @can('post-category-edit')
                                                    <a href="{{ route('dashboard.post.category.edit', $category->slug) }}" class="btn btn-warning btn-icon btn-xs me-1" title="UBAH DATA">
                                                        <i data-feather="edit"></i>
                                                    </a>
                                                @endcan
                                                @can('post-category-delete')
                                                    <form action="{{ route('dashboard.post.category.destroy', $category->slug) }}" method="POST" class="d-inline">
                                                        @method('DELETE')
                                                        @csrf
