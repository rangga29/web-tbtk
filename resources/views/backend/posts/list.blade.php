...
@section('content')
    <nav class="page-breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page"><a href="{{ route('dashboard.post.index') }}">Data Berita & Artikel</a></li>
        </ol>
    </nav>

    <div class="d-flex justify-content-between align-items-center grid-margin flex-wrap">
        <div>
            <h3 class="mb-md-0 fw-bolder text-uppercase mb-3">Data Berita & Artikel</h3>
        </div>
        @canany(['post-create', 'post-deleted'])
            <div class="d-flex align-items-center text-nowrap flex-wrap">
                @can('post-create')
                    <a href="{{ route('dashboard.post.create') }}" class="btn btn-primary btn-icon-text mb-md-0 me-2 mb-2">
                        <i class="btn-icon-prepend" data-feather="plus-square"></i>
                        Tambah Data
                    </a>
                @endcan
                @can('post-deleted')
                    <a href="{{ route('dashboard.post.deleted') }}" class="btn btn-danger btn-icon-text mb-md-0 mb-2">
                                    ...
                                    <tbody>
                                        @foreach ($posts as $post)
                                            <tr>
                                                ...
                                                <td><a href="{{ route('dashboard.post.show', $post->slug) }}">{{ $post->title }}</a></td>
                                                ...
                                                <td>{{ $post->isPublish == '0' ? 'Belum Publish' : 'Publish' }}</td>
                                                <td>{{ $post->publish_by != null ? $post->user_publish->name : '--' }}</td>
                                                <td>{{ $post->publish_at != null ? \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $post->publish_at)->isoFormat('DD MMMM Y HH:mm:ss') : '--' }}</td>
                                                @canany(['post-edit', 'post-delete'])
                                                    <td>
                                                        @can('post-edit')
                                                            <a href="{{ route('dashboard.post.edit', $post->slug) }}" class="btn btn-warning btn-icon btn-xs me-1" title="UBAH DATA">
                                                                <i data-feather="edit"></i>
                                                            </a>
                                                        @endcan
                                                        @can('post-delete')
                                                            <form action="{{ route('dashboard.post.delete', $post->slug) }}" method="POST" class="d-inline">
                                                                @method('DELETE')
                                                                @csrf
