...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('dashboard.links.store') }}" method="POST" class="forms-sample">
                        @csrf
                        <div class="row mb-3">
                            <label for="name" class="col-sm-2 col-form-label fw-bolder text-uppercase">Nama Link</label>
                            <div class="col-sm-10">
                                <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name" placeholder="Nama Link" value="{{ old('name') }}" autofocus>
                                @error('name')
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control @error('web_link') is-invalid @enderror" name="web_link" id="web_link" placeholder="Link Tujuan" value="{{ old('web_link') }}">
                                @error('web_link')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                        ...
                        <div class="d-flex justify-content-end">
                            ...
                            <a href="{{ route('dashboard.links.index') }}" class="btn btn-secondary fw-bolder">KEMBALI</a>
                        </div>
                    </form>
                </div>
            </div>
