...
@section('content')
    <nav class="page-breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page"><a href="{{ route('dashboard.about.entrepreneurship') }}">Entrepreneurship</a></li>
        </ol>
    </nav>

    <div class="d-flex justify-content-between align-items-center grid-margin flex-wrap">
        <div>
            <h3 class="mb-md-0 fw-bolder text-uppercase mb-3">Entrepreneurship</h3>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('dashboard.about.entrepreneurship.update') }}" method="POST" class="forms-sample" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <input type="hidden" name="name" value="{{ $entrepreneurship->name }}">
                        <div class="row mb-4">
                            ...
                            <div class="col-md-3">
                                <input type="hidden" name="oldBackground" value="{{ $entrepreneurship->background }}">
                                <img src="{{ asset('upload/' . $entrepreneurship->background) }}" class="card-img-top w-100" style="border-radius: 0px;" alt="Background">
                            </div>
                        </div>
                        <div class="row">
                            <label for="content" class="fw-bolder text-uppercase mb-2">Konten</label>
                            <div class="col-md-12">
                                <textarea class="form-control @error('content') is-invalid @enderror" name="content" rows="10">{{ old('content', $entrepreneurship->content) }}</textarea>
                                @error('content')
